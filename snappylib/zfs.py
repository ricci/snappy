...
def initCache():
    ...
    for line in iter(zfsout.splitlines()):
        ...
        zfsmap[path] = dataset
        zfsmap[dataset] = path
        if path in config.world.paths:
            zfsmap[config.world.paths[path].name] = dataset
    ...
    for line in iter(zfsout.splitlines()):
        ...
        if Snapshot.isSnappyZFS(snap) and dataset in zfsmap and zfsmap[dataset] in config.world.paths:
            newsnapshot = Snapshot.factory(config.world.paths[zfsmap[dataset]].name, Snapshot.isSnappyZFS(snap).group(1))
